...
function getBookstoRender(query) {
    if (!query) return Promise.resolve([...gBooks])
...
function addReview(review, bookId) {
    ...
    currBook={...currBook}
    currBook.review = (currBook.review) ? [...currBook.review, review] : [review]
    gBooks=gBooks.map(book=>currBook.id===bookId ? currBook:book) 
    //מה שעשיתי בשורה מעל - אם זה הספר שעדכנתי,  תביא את הספר שעדכנתי , אם  לא..תבחיא את הספרותיצור מערך חדש GBooks
    return Promise.resolve(currBook)
}

function deleteReview(bookId, reviewid) {
    // gBooks=gBooks.map(bookToFind=>{
    //     if(bookId)
    // })

    let currBook = gBooks.find(book => book.id === bookId)
    let reviews = currBook.review.filter(review => review !== reviewid)
    currBook.review = [...reviews]
...
function getBook(book){
    ...
    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${book}`)
...
function changePage(diff,bookId) {
    var bookIdx=gBooks.findIndex(book=>book.id===bookId)
    let location=bookIdx+diff
    console.log(location);
    console.log(gBooks)
    if (bookIdx+diff>gBooks.length) return Promise.resolve(gBooks[0].id)
    else if ((bookIdx+diff)<0) return Promise.resolve(gBooks[(gBooks.length-1)].id)
    else return Promise.resolve ((gBooks[location].id))
...
function createBook(newBook) {
    let bookDetails=  {
        ...
        "thumbnail":  newBook.volumeInfo.imageLinks.smallThumbnail,
...
let gBooks = [
    ...
    },
    {
        ...
        "authors": [
            "Barbara Cartland", "asdasd Cartland"
        ],
        "publishedDate": 1978,
